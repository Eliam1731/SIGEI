...
$sql = $conn->prepare("SELECT 
    ...
    INNER JOIN devolucion_de_equipos ON equipos_informaticos.Equipo_id = devolucion_de_equipos.Equipo_id
...
foreach ($rows as $row) {
    ...
    // Consulta para obtener los empleados que tuvieron a resguardo el equipo y lo devolvieron
    $sql_resguardos = $conn->prepare("
        SELECT 
            e.Nombre, 
            e.Primer_apellido, 
            e.Segundo_apellido, 
            ...
            r.status 
        FROM 
            empleados_resguardantes e 
        INNER JOIN 
            resguardos_de_equipos r 
        ON 
            e.Empleado_id = r.Empleado_id 
        WHERE 
            r.Equipo_id = ? AND 
            r.status = 'devuelto'
    ");
    $sql_resguardos->execute([$row['idEquipo']]);
    $resguardos = $sql_resguardos->fetchAll(PDO::FETCH_ASSOC);

    $equipment['resguardos'] = $resguardos;
